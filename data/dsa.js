...
const DSA_PROBLEMS = [
  {
    ...
    whyComplexity: "Time is O(n) because the array is scanned once and each hash-map insert/lookup is O(1) average; space is O(n) for the hash map, which in the worst case stores nearly every element before the matching pair is found.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(n) because each index is visited at most twice (once by the right pointer, at most once by the left) as the window slides; space is O(min(n, alphabet size)) for the last-seen-index map.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(n) because the two pointers only move inward and never revisit a position, so the total number of steps is bounded by the array length; space is O(1) since only a few index/variable trackers are kept.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(n * k log k) because each of the n strings is sorted independently in O(k log k) for length k; space is O(n * k) to store every string once inside the grouped output.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(1) amortized per operation because OrderedDict's move_to_end and popitem are both O(1) doubly-linked-list operations layered on O(1) average dict hashing; space is O(capacity) since the structure never holds more entries than the configured limit.",
    tests: [
  ...
  },
  {
    ...
    complexity: "O(n) time, O(1) space.",
    whyComplexity: "Time is O(n) because each node is visited and re-pointed exactly once; space is O(1) because the reversal happens in place with only a few pointer variables, independent of list length.",
    tests: [
  ...
  },
  {
    ...
    complexity: "O(n) time, O(1) space.",
    whyComplexity: "Time is O(n) because if a cycle exists the fast pointer gains one node on the slow pointer every step, guaranteeing a meeting within at most n steps, and if no cycle exists the fast pointer simply reaches the end; space is O(1) since only two pointers are used regardless of list length.",
    tests: []
  },
  {
    id: "dsa-08",
    ...
    complexity: "O(n) time, O(n) space.",
    whyComplexity: "Time is O(n) because each character is pushed or popped from the stack at most once; space is O(n) worst case if the string is all opening brackets, which all sit on the stack simultaneously.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(n) amortized because although there's a nested while loop, each index is pushed onto and popped off the deque at most once across the entire run, not once per outer iteration; space is O(k) since the deque never holds more than the current window's worth of candidate indices.",
    tests: [
  ...
  },
  {
    ...
    complexity: "O(n) time, O(n) space.",
    whyComplexity: "Time is O(n) because every node is enqueued and dequeued exactly once; space is O(n) worst case for a wide tree, since an entire level can be sitting in the queue at once.",
    tests: []
  },
  {
    id: "dsa-11",
    ...
    whyComplexity: "Time is O(rows * cols) because the DFS visits each cell at most once thanks to the in-place visited-marking (flipping '1' to '0'); space is O(rows * cols) worst case for the recursion stack if the entire grid is one connected landmass.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(V + E) because Kahn's algorithm processes each vertex once when its in-degree hits zero and each edge once when relaxing in-degrees; space is O(V + E) for the adjacency list plus the in-degree array and queue.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O((V + E) log V) because each edge relaxation can push a new entry onto the binary heap, and each heap operation costs O(log V); space is O(V) for the distance map plus O(E) worst case for heap entries.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is nearly O(n) (technically O(n * alpha(n)), the inverse Ackermann function) because path compression and union by rank keep each find/union operation nearly constant; space is O(n) for the parent and rank arrays.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(n log k) because each of the n elements triggers at most one O(log k) heap operation on a heap capped at size k; space is O(k) since the heap never grows past that cap.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(N log k) because there are N total elements across all lists, and each one triggers a single O(log k) push/pop on a heap that holds at most one entry per list (size k); space is O(k) for the heap.",
    tests: [
  ...
  },
  {
    ...
    complexity: "O(n) time, O(1) space.",
    whyComplexity: "Time is O(n) because each house is processed once in a single forward pass; space is O(1) because only the previous two running totals are kept, not a full DP array.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(m * n) because the DP table has m*n cells, each filled in O(1) from its already-computed neighbors; space is O(m * n) for the full table (reducible to O(min(m,n)) since each row only depends on the row above).",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(amount * len(coins)) because for every amount from 1 to the target, every coin denomination is tried once; space is O(amount) for the DP array indexed by amount.",
    tests: [
  ...
  },
  {
    ...
    complexity: "O(m * n) time and space.",
    whyComplexity: "Time is O(m * n) for the same reason as LCS — a full DP table where each cell is O(1) from its neighbors; space is O(m * n) for the table.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(2^n) because there are exactly 2^n subsets of an n-element set and each is built and appended once; space is O(2^n) to hold all of them in the output, plus O(n) recursion depth for the path.",
    tests: [
  ...
  },
  {
    ...
    whyComplexity: "Time is O(log n) because each comparison eliminates half the remaining search space, exactly like standard binary search, just with an extra check for which half is properly sorted; space is O(1) since only a few index variables are tracked.",
    tests: [
